...
module.exports = function (io) {
    ...
    host.on('connection', async socket => {
            ...
            socket.on('refresh:chartName', async (playersId, chartId, chartText=null) => {
                ...
                io.in([idRoom, `watchers:${idRoom}`]).emit('setAllGameData', data.emitData, {logsData:{type:`text`,value:textForLog}})

            })
        socket.on('stealChart', async (playerId1, playerId2,chartId) => {
            ...
            let textForLog = `Игрок ${await ioUserService.getNickname(playerId2)} украл характеристику у игрока ${await ioUserService.getNickname(playerId1)}`


            let data = await playerDataService.stealChart(playerId1,playerId2,gameRoom,chartName)
            io.to(`user:${playerId1}:${idRoom}`).emit('setAllGameData', data.players[playerId1])
            io.to(`user:${playerId2}:${idRoom}`).emit('setAllGameData', data.players[playerId2])
            await UserModel.Logi.create({
                idRoom: idRoom,
                funcName: `stealChart`,
                text: textForLog,
        ...
        socket.on('addChart', async (playersId,chartId,chartText=null) => {
            ...
            for(let player of players){
                ...
                if(data.isOpen){
                    ...
                    io.to(`user:${player.userId}:${idRoom}`).emit('setAllGameData', {players:{[player.userId]:{[chartName]:data}}})
                }
            }
            await UserModel.Logi.create({
                idRoom: idRoom,
        ...
        socket.on('deleteRelocate', async (playersId,makeId) => {
            let arrayMake = ['Удалить инвентарь','Перенести инвентарь']
            if(playersId===0 && makeId ===0){
                socket.emit("setError",
                    {
                        message: "Нельзя перенести инвентарь в бункер всем игрокам",
                        status: 400,
            ...
            for(let player of players){
                if(makeId===0){
                    ...
                    if(data.isOpen){
                        ...
                        io.to(`user:${player.userId}:${idRoom}`).emit('setAllGameData', {players:{[player.userId]:{inventory:data}}})
                    }
                }else{
                    let data = JSON.parse(player.inventory)
            ...
            if(playerId1===0){
                ...
                for(let player of players){
                    if(data.players[player.userId]){
                        io.to(`user:${player.userId}:${idRoom}`).emit('setAllGameData', {players:{[player.userId]:{[chartName]:data.players[player.userId]}}})
                    }
                }
               ...
               textForLog=`Ведущий совершил обмен характеристикой между игроком ${await ioUserService.getNickname(playerId1)} и игроком ${await ioUserService.getNickname(playerId2)}`
                let player1 = await UserModel.RoomSession.findOne({
                ...
                if(playerData1.isOpen){
                    ...
                    io.to(`user:${playerId1}:${idRoom}`).emit('setAllGameData', {players:{[playerId1]:{[chartName]:playerData1}}})
                }
                if(playerData2.isOpen){
                    emitData.players[playerId2]={[chartName]:playerData2}
                }else{
                    io.to(`user:${playerId2}:${idRoom}`).emit('setAllGameData', {players:{[playerId2]:{[chartName]:playerData2}}})
                }
        ...
        socket.on('transferHost', async (playerId) => {
            ...
            let textForLog = `Ведущий передал свои права игроку ${ioUserService.getNickname(playerId)} `
            ...
            io.in([idRoom, `watchers:${idRoom}`]).emit('setAllGameData', {logsData:{type:
        ...
        socket.on('refresh:ByHour', async (chartId,makeId) => {
            ...
            if(makeId===0) {
                ...
                if (playerProfessionData.isOpen) {
                    ...
                    io.to(`user:${players[players.length - 1].userId}:${idRoom}`).emit('setAllGameData',
            ...
            await UserModel.Logi.create({
                idRoom: idRoom,
                funcName: `ByHour`,
                text: `Ведущий изменил ${chartName} ${makeName}`,
                step: await playerDataService.howStepLog(idRoom),
                lastVar: JSON.stringify(lastVar)
            })
            emitData.logsData.type = 'text'
            emitData.logsData.value = `Ведущий изменил ${chartName} ${makeName}`
            io.in([idRoom, `watchers:${idRoom}`]).emit('setAllGameData', emitData)
            io.in(idRoom).emit('refresh:professionByHour:good')
        })
